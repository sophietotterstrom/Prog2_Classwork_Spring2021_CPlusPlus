...
int main() {
    ...
    while ( true ) {
        ...
        {
            new_command.push_back(toupper(character));
        }
        ...
        {
            if (COMMANDS[i].str == new_command)
            {
                if (pieces.size()-1 != COMMANDS[i].parameter_number) {
                ...
                double double1;
                string_to_double(pieces.at(1), double1);
                double double2;
                string_to_double(pieces.at(2), double2);

                if (not string_to_double(pieces.at(2), double2)
                        or not string_to_double(pieces.at(1), double1)){

                    cout << "Error: a non-number operand." << endl;
                    break;
                }

                double result = COMMANDS[i].action(double1,double2);
                cout << result << endl;

                break;
            }
            if ((i == (int)COMMANDS.size()-1) and (COMMANDS[i].str != new_command))
            {
